...
if(args[2] == "subsample"){
	outfile.arms <- paste(c("_alphaOmega_subsamp_ARMS_gen", as.character(args[1])), collapse="")
	make.est_alpha_omega.input(poly.dat=pdat.arms, genome.dat=gdat.arms, fixed.dat=fdat.arms, 
	generation=gen, num.inds.sampled=inds.sampled, genome.size=gsize.arms, 
	filename=outfile.arms, use.manual.sample=TRUE)
        outfile.centers <- paste(c("_alphaOmega_subsamp_CENTERS_gen", as.character(args[1])), collapse="")
        make.est_alpha_omega.input(poly.dat=pdat.centers, genome.dat=gdat.centers, fixed.dat=fdat.centers,
        generation=gen, num.inds.sampled=inds.sampled, genome.size=gsize.centers,
        filename=outfile.centers, use.manual.sample=TRUE)
}else{
	outfile.arms <- paste(c("_alphaOmega_full_ARMS_gen", as.character(args[1])), collapse="")
	make.est_alpha_omega.input(poly.dat=pdat.arms, genome.dat=gdat.arms, fixed.dat=fdat.arms, 
	generation=gen, num.inds.sampled=pop.size, genome.size=gsize.arms, 
	filename=outfile.arms, use.manual.sample=FALSE)
        outfile.centers <- paste(c("_alphaOmega_full_CENTERS_gen", as.character(args[1])), collapse="")
        make.est_alpha_omega.input(poly.dat=pdat.centers, genome.dat=gdat.centers, fixed.dat=fdat.centers,
        generation=gen, num.inds.sampled=pop.size, genome.size=gsize.centers,
        filename=outfile.centers, use.manual.sample=FALSE)
}
