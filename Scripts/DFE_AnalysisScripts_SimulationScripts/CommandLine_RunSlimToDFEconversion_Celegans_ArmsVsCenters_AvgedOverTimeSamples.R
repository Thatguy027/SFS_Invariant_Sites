...
fdat <- read.table(paste(c("FixedOutput_", paste(unlist(strsplit(as.character(args[1]), split="_")), collapse="_")), collapse=""), skip=fixed.mut.id.start)
...
if(args[2] == "subsample"){
    outfile.arms <- paste(c("DFE_SFS_subsamp_ARMS_gen", gen, "_", as.character(args[1])), collapse="")
    ...
    outfile.centers <- paste(c("DFE_SFS_subsamp_CENTERS_gen", gen, "_", as.character(args[1])), collapse="")
    ...
    outfile.arms <- paste(c("DFE_SFS_full_ARMS_gen", gen, "_", as.character(args[1])), collapse="")
    make.est_dfe.input(poly.dat=pdat.arms, genome.dat=gdat.arms, fixed.dat=fdat.arms,
    generation=gen, num.inds.sampled=pop.size, genome.size=gsize.arms,
    filename=outfile.arms, fold=TRUE, use.manual.sample=FALSE)
    outfile.centers <- paste(c("DFE_SFS_full_CENTERS_gen", gen, "_", as.character(args[1])), collapse="")
    make.est_dfe.input(poly.dat=pdat.centers, genome.dat=gdat.centers, fixed.dat=fdat.centers,
...
outfile.centers <- paste(c("Avg_SFS_10NtoEnd_CENTERS_", as.character(args[1])), collapse="")
outfile.arms <- paste(c("Avg_SFS_10NtoEnd_ARMS_", as.character(args[1])), collapse="")
files.centers <- system(paste(c("ls Folded*CENTERS*", as.character(args[1])), collapse=""), intern=TRUE)
files.arms <- system(paste(c("ls Folded*ARMS*", as.character(args[1])), collapse=""), intern=TRUE)
sfs.dat.centers <- vector("list", length(gens))
sfs.dat.arms <- vector("list", length(gens))
for(j in 1:length(gens)){
    sfs.dat.centers[[j]] <- as.matrix(read.table(files.centers[j], skip=2))
    sfs.dat.arms[[j]] <- as.matrix(read.table(files.arms[j], skip=2))
}
avged.sfs.dat.centers <- apply(simplify2array(sfs.dat.centers), MARGIN=1:2, FUN=mean)
avged.sfs.dat.arms <- apply(simplify2array(sfs.dat.arms), MARGIN=1:2, FUN=mean)

dfe.input.centers <- paste(c(
'1
', sample.size,'
', paste(c(avged.sfs.dat.centers[1,]), collapse=" "),'
', paste(c(avged.sfs.dat.centers[2,]), collapse=" ")
), collapse="")

write(dfe.input.centers, file=outfile.centers)

dfe.input.arms <- paste(c(
'1
', sample.size,'
', paste(c(avged.sfs.dat.arms[1,]), collapse=" "),'
', paste(c(avged.sfs.dat.arms[2,]), collapse=" ")
), collapse="")

write(dfe.input.arms, file=outfile.arms)
