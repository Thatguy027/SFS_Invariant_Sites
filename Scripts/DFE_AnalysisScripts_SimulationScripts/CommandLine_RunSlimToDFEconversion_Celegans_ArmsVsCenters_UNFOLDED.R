...
if(args[2] == "subsample"){
    outfile.arms <- paste(c("DFE_SFSunfolded_subsamp_ARMS_gen", as.character(args[1])), collapse="")
    ...
    filename=outfile.arms, fold=FALSE, use.manual.sample=TRUE)
    outfile.centers <- paste(c("DFE_SFSunfolded_subsamp_CENTERS_gen", as.character(args[1])), collapse="")
    ...
    filename=outfile.centers, fold=FALSE, use.manual.sample=TRUE)
}else{
    outfile.arms <- paste(c("DFE_SFSunfolded_full_ARMS_gen", as.character(args[1])), collapse="")
    make.est_dfe.input(poly.dat=pdat.arms, genome.dat=gdat.arms, fixed.dat=fdat.arms,
    generation=gen, num.inds.sampled=pop.size, genome.size=gsize.arms,
    filename=outfile.arms, fold=FALSE, use.manual.sample=FALSE)
    outfile.centers <- paste(c("DFE_SFSunfolded_full_CENTERS_gen", as.character(args[1])), collapse="")
    make.est_dfe.input(poly.dat=pdat.centers, genome.dat=gdat.centers, fixed.dat=fdat.centers,
    generation=gen, num.inds.sampled=pop.size, genome.size=gsize.centers,
    filename=outfile.centers, fold=FALSE, use.manual.sample=FALSE)
}
