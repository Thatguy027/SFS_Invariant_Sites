...
if(args[2] == "subsample"){
	outfile <- paste(c("DFE_SFSunfolded_subsamp_gen", as.character(args[1])), collapse="")
	...
	filename=outfile, fold=FALSE, use.manual.sample=TRUE)
}else{
	outfile <- paste(c("DFE_SFSunfolded_full_gen", as.character(args[1])), collapse="")
	make.est_dfe.input(poly.dat=pdat, genome.dat=gdat, fixed.dat=fdat, 
	generation=gen, num.inds.sampled=pop.size, genome.size=gsize, 
	filename=outfile, fold=FALSE, use.manual.sample=FALSE)
}
