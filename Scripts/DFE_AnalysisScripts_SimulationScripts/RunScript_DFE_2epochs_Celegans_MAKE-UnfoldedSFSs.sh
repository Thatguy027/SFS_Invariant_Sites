...
ls $dir | grep Modified | sed "s/ModifiedSampleOutput_//g" > base_InputNames2.txt

# loop through the list and do each one at a time
for base_name in `cat base_InputNames2.txt`
do
# then analyze like normal:
Rscript CommandLine_RunSlimToDFEconversion_Celegans_UNFOLDED.R $base_name subsample $dir $genosize $popsize
# create divergence file (for all of them because easier to do in this loop anyway
Rscript CommandLine_RunSlimToAlphaOmega_Celegans.R $base_name subsample $dir $genosize $popsize
done
